...
function FormAds() {
  ...
  const handleUploadFileVideo = (event) => {
    ...
    const selectedVideos = Array.from(event.target.files).filter((file) =>
  ...
  return (
    <div className=" relative">
      <div className="bg-cover px-auto py-20">
        <h2 className="mt-5 mb-3 font-bold text-xl text-start ml-20">
          GỬI THÔNG TIN QUẢNG CÁO
        </h2>
      </div>

      <form
        className="space-y-6 border-2 p-6 max-w-[1300px] m-auto relative pt-10 bg-white rounded-lg"
      ...
      >
        <div className="rounded-md shadow-sm">
          ...
          <div className="flex gap-3 w-full pb-5">
            ...
            {isAlertLink && (
              <SentWarning title="Bạn đã đạt giới hạn của việc thêm tin !" />
            )}
          </div>
          ...
          {isAlertImage && (
            <SentWarning title="Không thể upload quá 10 ảnh !" />
          )}
          <div className="custom-file py-3 border rounded px-2 mb-3">
            <input
              type="file"
              multiple
              accept="video/*"
              onChange={handleUploadFileVideo}
            />
          </div>
          {isAlertVideo && (
            <SentWarning title="Không thể upload quá 2 video !" />
          )}
          <button
            type="submit"
            className="py-3 w-full bg-yellow-600 text-white font-rob mt-4 rounded-lg"
          >
            GỬI ĐI
          </button>
        </div>
